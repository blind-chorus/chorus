...
const AGENT_PRINCIPLES = [
  ...
  <>
    <strong>Component flexibility — extrapolate, don&apos;t fork.</strong> Chorus components ship with reference compositions and per-spec guidelines. Read the intent and respect each component&apos;s anatomy invariants, but feel free to flex how it&apos;s composed (slot fill, layout placement, modifier props) to fit a specific UI/UX context. The contract is the token bindings and the spec&apos;s slot/sizing rules, not the example screenshot. The family&apos;s <code>visualReuse</code> flag says how far that flexibility extends — <em>open</em> families (most of the system) may be reached for on <strong>visual-fit grounds even when the brief&apos;s intent does not match <code>useCases</code> verbatim</strong>; <em>locked</em> families (<code>dialog</code>, <code>bottom-sheet</code>, <code>toast</code>, <code>tooltip</code>, <code>form-field</code>) MUST only be used in their canonical role because the interaction contract is the point.
  </>,
  <>
  ...
  <>
    <strong>UX-pattern consistency.</strong> Pick components from the interaction the user expects when that interaction is the whole point — Dialog for modal commits, BottomSheet for committed-sheet flows, Toast for non-blocking feedback, Tooltip for trigger-anchored hints, FormField for real text entry. These five families are <code>visualReuse: &quot;locked&quot;</code> and MUST NOT be borrowed for the visual shape alone — focus trap, auto-dismiss, ARIA live region, hover/focus trigger, and <code>&lt;input&gt;</code> semantics are the contract. The other thirteen families (<code>visualReuse: &quot;open&quot;</code>) carry interaction defaults too — List for menus/pickers, Feed for authored content, Chip vs Button for facet vs commit — but those defaults are <em>first suggestions</em>, not rules; the agent may pick by visual fit when the design calls for it. Across a single flow, keep behavior, motion, and affordance language predictable regardless of which tier the components come from.
  </>,
];
...
const COMPONENT_INDEX = [
  ...
  ['suggestion-list',  'Channel List',  'A swipeable pager of channel suggestions — three rows per page (Thumbnail 48 + name / followers / description + Toggle Button), with the next page peeking to invite the swipe.'],
  ['avatar-rail',  'Channel Rail',  'A horizontally-scrolling rail of subscribed channels — circular thumbnail + name, with an edge fade indicating off-screen items and an optional trailing "View all" action.'],
