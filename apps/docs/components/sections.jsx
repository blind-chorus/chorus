  ...
  return (
    ...
    >
      ...
      <ProseSection title="Base-unit ladder">
        ...
        <ul className="rule-list">
          <li><strong>Spacing</strong> binds it to <strong>pixels</strong> — each rung is a <code>ref.space.*</code> step (<code>0px</code> … <code>80px</code>); see <a href="/spacing#spacing-reference">Reference scale</a> for the per-step table.</li>
    ...
    >
      <ProseSection>
        <ul className="rule-list">
          ...
          <li><strong>Shadow alphas come from the overlay palettes.</strong> <code>ref.palette.black.*</code> draws from the <a href="/spacing#base-unit-ladder">base-unit ladder</a> read as percent — every shadow alpha (4 / 6 / 8 / 12 / 16 / 20%) is a step on that ladder, so a step rebalance in the palette propagates through every elevation preset without re-authoring shadows.</li>
        </ul>
      </ProseSection>
      <div className="specimen-grid">
        {items.map(([key, token]) => (
...
function ResponsiveTouchTargets() {
  return (
    <Section
      ...
      description={
        <>Tap-target sizing is owned by <a href="/accessibility#a11y-targets">Accessibility › Touch &amp; Pointer targets</a>; §Foundations does not redefine it. The same <code>layout.container.sm</code> (12 → 16px padding) default produces a 40–48px control height that already clears the 44px iOS / 48px Material guideline on mobile.</>
      }
    />
  );
}
...
const COMPONENT_INDEX = [
  ...
  ['button',        'Button',        'Standard inline button (three sizes × four appearances) plus three sub-component families: FAB (canvas-anchored floating commit), Toolbar Button (dense inline action), and Toggle Button (reversible commit — Follow ↔ Following).'],
  ...
  ['feed',          'Feed',          'A vertically-stacked content card — channel header, post block (title / body / thumbnail), and optional inline modules (poll, citation, mention) above an engagement footer.'],
  ['form-field',    'Form Field',    'Text-entry primitives — Input (single-line field, hairline outline at rest, heavier focus ring, optional clear button, error re-tone) and Search (the same field with a leading SearchIcon and a pill `radius.full` corner).'],
