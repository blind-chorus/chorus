...
- Use `button / check` for the rows, not `list / radio` or a custom checkbox: rows commit option state alongside the sheet's primary action, which is exactly Check Button's intent (see [button/check](../components/button/check.md)).
...
- No paired screen recipe yet — when one is added under `screens/`, link both ways and update the index row's Recipe column.
