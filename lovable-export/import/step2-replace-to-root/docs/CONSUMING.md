...
Component-level props (`variant`, `appearance`, `size`, slots) follow the per-component spec in `components/`. The docs site renders these specs as the authoritative reference.
...
Token regeneration after editing `tokens/*.json`:
...
A drift check (`node packages/tokens-{ios,android}/scripts/check.mjs`) runs in CI and exits non-zero if `tokens/*.json` changed without the matching `Generated/*` regenerated — keeping the three implementations (web, iOS, Android) in lockstep.

Pilots shipped so far: Button (full spec) and Chip (filter + tag). The remaining components in `components/` are not yet ported.
