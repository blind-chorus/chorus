...
Pre-validated screen-level compositions. Each `*.screen.json` binds regions (`top`/`body`/`bottom`/`fab`/`overlay`) to families + sub-components from `components/`, with concrete demo data inline.
