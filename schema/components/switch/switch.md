...
A binary on/off control — a pill-shaped track with a circular thumb that translates between ends. Off reads as a muted track with an `outlineVariant` hairline; on paints the track in `primary` so the contract reads chromatically without an inline label.
...
The bare Switch — click or press Space / Enter to toggle.
...
Fades both on and off to `sys.state.disabled` and freezes the thumb. Non-interactive.
...
- **track** — pill container, 52 × 32, fully rounded. Carries click, focus ring, and overlay paint. `role="switch"`; `aria-checked` reflects on/off.
...
| thumb (off)   | `sys.color.onSurfaceVariant` fill, 28 × 28, fully rounded, 2px inset from leading edge |
...
A single appearance — no emphasis axis. The visible variation is the `data-state="on"` / `"off"` contract.
