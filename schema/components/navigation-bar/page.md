...
The drill-in top bar — anchored to every screen one step inside a flow. Centred page name, flanked by an optional leading slot (typically back-arrow) and a trailing slot (a [Toolbar Button](../button/toolbar.md), text link, or single icon). Title drops from Home's `typo.heading.lg` to `typo.heading.sm` (16/Semibold).

**Reach for this when** the screen is one step inside a flow and needs a back affordance plus a single primary action. **Skip when** you're on a tab root (use [Home](./home.md)) or a dedicated search page (use [Search](./search.md)).
