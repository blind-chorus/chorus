...
- **avatar** — [Thumbnail](../thumbnail/thumbnail.md) `size={56}` overlapping the cover band's bottom edge by half its diameter (`margin-top: -28px`). Carries a 2-token (`sys.borderWidth.thin`) outset border in the host's surface tone via `box-shadow`, separating the circle from the cover.
...
| avatar        | [Thumbnail](../thumbnail/thumbnail.md) `size={56}`, `margin-top: -28px` (vertical centre on cover bottom edge), `box-shadow: 0 0 0 sys.borderWidth.thin sys.color.surface` (2-token outset halo) |
