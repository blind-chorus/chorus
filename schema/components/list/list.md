...
> Inherits the Chorus-wide rules in [`DESIGN.md`](../../DESIGN.md) — color quartets, state overlays, focus ring composition, type scale, surface ladder. Per-sub anatomy lives in the sibling specs below.
...
- **State recipes.** Default / hovered / pressed / disabled compose via [State overlays](../../DESIGN.md#state-overlays) on the row foreground. `selected` exists only on Radio. The whole row is the interactive surface.
- **Focus indicator.** Composition: Inward (see [DESIGN.md → Focus ring composition](../../DESIGN.md#focus-ring-composition)) — inset shadows entirely inside the row's box. Trigger: `:focus-visible`.
