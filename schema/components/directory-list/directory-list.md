...
**Reach for this when** the surface should expose the *whole* follow-able set (browse, new channels, people you may know) and each row should commit a follow in place. **Skip when** the surface is a peek of three (use [SuggestionList](../suggestion-list/suggestion-list.md)) or the rows are not follow-able routes (use [List](../list/entry.md) directly).
