...
A bar including a primary-coloured Create affordance at the trailing end. The icon ([`PlusSquareFillIcon`](../../packages/ui/src/icons/svg/AddSquareFill.svg)) is painted in `sys.color.brand` via `appearance="primary"` and still occupies one equal-width slot.
