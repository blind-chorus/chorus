...
- **[`prompt/`](prompt)** — agent-facing prompt sources, grouped together for humans browsing the repo and for trivial Finder drag-and-drop into chat-based agents.
  - [`prompt/DESIGN_PRINCIPLES.md`](prompt/DESIGN_PRINCIPLES.md) — Lovable agent persona (system-prompt source + user-turn preamble). Same file is published inside `@blind-dsai/ui/agents/DESIGN_PRINCIPLES.md` for npm consumers (kept in sync by `packages/ui/scripts/copy-agents.mjs`). The repo-root [`AGENTS.md`](AGENTS.md) stays at root so Cursor / Claude Code / Lovable can auto-discover it.
