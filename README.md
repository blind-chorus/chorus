...
- **[`prompt/`](prompt)** — agent-facing prompt sources, grouped together for humans browsing the repo and for trivial Finder drag-and-drop into chat-based agents.
  - [`prompt/LOVABLE.md`](prompt/LOVABLE.md) — Lovable-specific agent persona (system-prompt source + user-turn preamble). Same file is published inside `@blind-dsai/ui/agents/LOVABLE.md` for npm consumers (kept in sync by `packages/ui/scripts/copy-agents.mjs`). The repo-root [`AGENTS.md`](AGENTS.md) stays at root as the *universal* machine contract every Chorus-aware agent obeys — Cursor / Claude Code / Lovable all auto-discover it.
